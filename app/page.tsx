...
const regionCompounding: Record<string, number> = {
  US: 12,
  EU: 1,
  Canada: 2,
  Australia: 12,
...
export default function HomePage() {
  ...
  const monthlyPayment = useMemo(() => {
    const compoundingPeriods = regionCompounding[region] ?? 12;
    const periodicRate = interestRate / 100 / compoundingPeriods;
    const monthlyRate = Math.pow(1 + periodicRate, compoundingPeriods / 12) - 1;
  ...
  }, [interestRate, loanAmount, loanTerm, region]);
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col space-y-12 rounded-[28px] border border-slate-300 bg-white/90 p-6 shadow-lg shadow-[0_25px_60px_rgba(15,23,42,0.12)] backdrop-blur-xl sm:p-10">
        <header className="flex flex-col gap-5 text-center">
          ...
          <div className="space-y-3">
            <h1 className="text-4xl font-bold leading-tight text-slate-900 sm:text-6xl sm:leading-tight">
              Global mortgage estimator for confident planning
            </h1>
          ...
          <span className="text-sm text-slate-700">Estimates only — not financial advice.</span>
        </header>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.05fr,0.95fr]" id="calculator">
          <section className="relative overflow-hidden rounded-3xl border border-slate-300 bg-white p-6 shadow-lg shadow-slate-200/70 backdrop-blur-xl sm:p-8">
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-900" htmlFor="region">
                  Compounding region
                </label>
                <select
                  id="region"
                  ...
                  className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-base text-slate-900 shadow-inner shadow-slate-100 outline-none transition duration-300 hover:-translate-y-0.5 hover:border-blue-500 hover:shadow-[0_12px_30px_rgba(56,189,248,0.18)] focus:-translate-y-0.5 focus:border-emerald-300 focus:ring-4 focus:ring-blue-200"
                >
                  <option value="US">United States — monthly compounding</option>
                  <option value="EU">European Union — annual compounding</option>
                  <option value="Canada">Canada — semi-annual compounding</option>
                  <option value="Australia">Australia — monthly compounding</option>
                </select>
                <p className="text-xs text-slate-600">
                  Adjusts the effective monthly rate based on common compounding practices in your region.
                </p>
              </div>
